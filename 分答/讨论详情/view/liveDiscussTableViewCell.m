...
-(void)creatUI
{
    ...
    _sutUpBtn=[UIButton buttonWithType:UIButtonTypeCustom];
    [_sutUpBtn setImage:[UIImage imageNamed:@"prohibition_of_speaking"] forState:UIControlStateNormal];
    [self.contentView addSubview:_sutUpBtn];
    ...
    [_sutUpBtn mas_makeConstraints:^(MASConstraintMaker *make) {
...
-(void)setDiscussModel:(liveDiscussModel *)discussModel
{
    ...
    _sutUpBtn.hidden=YES;
    if (_discussModel.hasShutUp) {
        _sutUpBtn.hidden=NO;
    }
    _timeLabel.text=[_discussModel create_time:_discussModel.createTime];
    
    if (_discussModel.replyCreatorId>0) {
        NSMutableAttributedString*atrributStr=[[NSMutableAttributedString alloc]initWithString:[NSString stringWithFormat:@"@%@ %@",_discussModel.replyCreatorName,_discussModel.content]];
        [atrributStr addAttribute:NSForegroundColorAttributeName value:themeColor range:NSMakeRange(0, _discussModel.replyCreatorName.length+1)];
        _contentLabel.attributedText=atrributStr;
    }
    else
    {
        _contentLabel.text=_discussModel.content;
    }
