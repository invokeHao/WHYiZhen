//
//  MytopicViewTableViewCell.m
//  Yizhenapp
//
//  Created by augbase on 16/6/21.
...
@implementation MytopicViewTableViewCell
{
...
- (void)setModel:(topicModel *)model
{
    ...
    [_iconButton sd_setBackgroundImageWithURL:[NSURL URLWithString:_model.creatorAvatar] forState:UIControlStateNormal placeholderImage:[UIImage imageNamed:@"default_avatar3"]];
    _nameLabel.text=model.creatorName;
    _TimeLabel.text=[model create_time:model.createTime];
    YiZhenLog(@"%@",model.createTime);
    YiZhenLog(@"%@",model.updateTime);
    ...
    if (model.attachmentList.count>0) {
        ...
        webUrlArr=[[NSMutableArray alloc]initWithCapacity:0];
        typeArr=[[NSMutableArray alloc]initWithCapacity:0];
        ...
        for (attachmentListModel*attachModel in model.attachmentList) {
            ...
            if(attachModel.type==0)
            {
                ...
                [webUrlArr addObject:webUrl];
                [typeArr addObject:[NSString stringWithFormat:@"%ld",attachModel.type]];
                
                linkButton*linkBtn=[[linkButton alloc]initWithFrame:CGRectMake((22+50)*j, 0, 50, 30) andType:attachModel.type];
                linkBtn.tag=100+j;
...
-(void)GotoOcrWebView:(UIButton*)button
{
    switch (button.tag) {
        case 100:
            _webB(webUrlArr[0],[typeArr[0] integerValue]);
            break;
        case 101:
            _webB(webUrlArr[1],[typeArr[1] integerValue]);
        default:
            break;
    }
}
